...
\caption{Characteristics of data sets used in the experiments.}
 \label{table-datasets}
  ...
  \begin{tabular}{ccccc}
  \hline\noalign{\smallskip}
  Data set~ & ~Number of~ & ~Number of true~ & ~Linkage~ & ~Entities\\
  name(s)  & records  & matching pairs & type & linked \\
  \noalign{\smallskip} \hline \noalign{\smallskip}
  CORA  & 1,295 & 17,184 & many-many & publication-publication\\
  NCVR  & ~448,134~ & ~148,036~ & 1:1  & voter-voter\\
  Isle of Skye & 32,567 & 2,900 & 1:1 & birth-death\\
  Kilmarnock  & 70,852 & 8,300 & 1:1 & birth-death\\
  \noalign{\smallskip} \hline
  \end{tabular}
  \end{scriptsize}
\end{table}
...
%The experiments for each technique have been run within a generic Java framework which permits the actual linkage method used to be supplied. This ensures that the algorithms are consistently executed and that true comparisons of the their behaviour can be reported. It supplies each algorithm with the source records to be matched using an \emph{addSourceRecord} method which takes a record as a parameter. For each of the records to be matched a  \emph{getCandidateMatches} method is called which is expected to return a list of the linked records from the sources. Each of our datasets contains ground truth with which we can establish the records that are true links and those that are not (discussed below). From this ground truth the framework can report the number of links make, linkage quality etc. 
