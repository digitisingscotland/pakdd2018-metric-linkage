...
% Peter: remove probabilistic - hat we do is general to RL, the
% probabilistic aspect is not really relevant
...
% Peter: are classified to be link (i.e.\ refer to the same entity)
...
% Peter: $O(n^2)$, where $n$ is the number of records in the dataset to
% be linked.
used, this approach will find it.
% Peter: find -> identify  -- better actually clarify: This approach
% will identify all links of records that have a distance up-to the
% given distance threshold, $d$.
...
% peter: below fast -> I would remove fast, as it might be slow for
...
% Peter: run through.. -> reword: is converted into such a LSH
% signature then..  records with the same key(s) are...
% Peter: close to.. -> is a bit vague
...
% peter: we will evaluate the performance of LSH based blocking with
% regard to the three parameters $l_{ss}$, $l_{bs}$ and $l_{nb}$ in
% Sect.~\ref{sec-exp}.
% peter: below - we want to do record linkage (slightly different
% from nn search) - better to describe RL below not NN search! 
...
% peter: (distances must support triangle inequality, i.e.\ for three
% objects $i$, $j$ and $k$, $dist(i,j) + dist(j,k) \ge dist(i,k)$).
%
...
% peter: where all objects $i$ within a distance $d$ for a query object
% $q$ are identified, i.e.\ $dist(i,q) \le d$)
\textit{nearest-neighbour} (the closest value to q), and \textit{nearestN} (the N closest n objects to q).
% peter: we can add proper mathematical ways of defining those if
% needed
An M-tree is a balanced tree structure comprised of fixed sized nodes that can each store up to \textit{M} children.
% not sure if we should use M here - the M in M-tree is not because
% each node can have m children! In Sect 5 I used $t_b$ as name for
...
% Peter: and how is an M-tree used in record linkage?
...
% peter: could we have some pseudo-code descriptions of how these
% techniques are used for RL blocking?
...
% Peter: Why same number of links for Skye and Killi? *****************
\fi
...
% the narrow values for ki and ks need some justification!
